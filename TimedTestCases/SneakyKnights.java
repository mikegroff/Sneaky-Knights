...
public class SneakyKnights
{
  ...
  public static boolean allTheKnightsAreSafe(ArrayList<String> coordinateStrings, int boardSize)
  {
    ...
    for(String knights : coordinateStrings)
    {
      ...
      {
        if(UsedKnights.get(j) != null)//sees if another queen can attack for each of the 9 keys
          return false;
        else
          UsedKnights.put(KeyLocations.get(0), 1);// if not places these keys in the map to check if other queens can attack
      }
    }
